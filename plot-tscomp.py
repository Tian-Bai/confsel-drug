...
for name in dataset_list:
    ...
    for j in range(1, 1+n_itr):
        try:
            df = pd.read_csv(os.path.join("result_ts", f"{sample:.2f}", f"{name} {sample:.2f}", f"{name} {sample:.2f} {j}.csv"))
...
for i, name in enumerate(dataset_list):
    ...
    if i == 0:
        ...
        line2, = ax.plot(df_list[i]['fdp_nominals'], df_list[i]['fdps_bin_c'], 
                        label='binarization + classification', marker='o', color='orange', alpha=0.5)
        
        line3, = ax.plot(df_list[i]['fdp_nominals'], df_list[i]['fdps_neg'], 
                        label='negation', marker='o', color='darkgreen', alpha=0.8)
        ...
        ax.plot(df_list[i]['fdp_nominals'], df_list[i]['fdps_bin_c'], 
                marker='o', color='orange', alpha=0.5)
        
        ax.plot(df_list[i]['fdp_nominals'], df_list[i]['fdps_neg'], 
...
# Display the plot
plt.savefig(os.path.join("pic", "fdp_ts.pdf"))
# plt.show()
...
for i, name in enumerate(dataset_list):
    ...
    if i == 0:
        ...
        line2, = ax.plot(df_list[i]['fdp_nominals'], df_list[i]['powers_bin_c'], 
                        label='binarization + classification', marker='o', color='orange', alpha=0.5)
        
        line3, = ax.plot(df_list[i]['fdp_nominals'], df_list[i]['powers_neg'], 
                        label='negation', marker='o', color='darkgreen', alpha=0.8)
        ...
        ax.plot(df_list[i]['fdp_nominals'], df_list[i]['powers_bin_c'], 
                marker='o', color='orange', alpha=0.5)
        
        ax.plot(df_list[i]['fdp_nominals'], df_list[i]['powers_neg'], 
                marker='o', color='darkgreen', alpha=0.8)
...
# Display the plot
plt.savefig(os.path.join("pic", "power_ts.pdf"))
# plt.show()