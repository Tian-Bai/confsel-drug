...
threshold_path = os.path.join('data_threshold', f'{dataset_name}_threshold.csv')
...
total_threshold = pd.read_csv(threshold_path)['threshold'].to_numpy()

total_Y = dataset['Act'].to_numpy()
total_X = dataset.drop(columns=['MOLECULE', 'Act']).to_numpy()

Xtc, Xtest, Ytc, Ytest, Ctc, Ctest = train_test_split(total_X, total_Y, total_threshold, test_size=15/100, shuffle=True) # split 15% as the test data, and the rest for train and calib (tc)
...
Xtrain, Xcalib, Ytrain, Ycalib, Ctrain, Ccalib = train_test_split(Xtc, Ytc, Ctc, train_size=50/85, shuffle=True)
rf.fit(Xtrain, Ytrain < Ctrain)

fdps_bin_r, pcers_bin_r, powers_bin_r = [], [], []

for i, fdp_nominal in enumerate(fdp_nominals):
    calib_scores = 1000 * (Ycalib < Ccalib) - rf.predict(Xcalib)
    test_scores = -rf.predict(Xtest)

    pvals = conf_pval(calib_scores, test_scores)
    sel = BH(pvals, fdp_nominal)
    fdp, pcer, power = eval(Ytest, sel, -np.inf, Ctest)
...
for i, fdp_nominal in enumerate(fdp_nominals):
    calib_scores = 1000 * (Ycalib < Ccalib) - rf.predict_proba(Xcalib)[:, 1] # probability of 1
...
# save the results
out_dir = os.path.join('result_varying_c', f'{dataset_name} {args.sample:.2f}')
if not os.path.exists(out_dir):
    os.makedirs(out_dir)

all_res.to_csv(os.path.join('result_varying_c', f'{dataset_name} {args.sample:.2f}', f'{dataset_name} {args.sample:.2f} {args.seed}.csv'))
